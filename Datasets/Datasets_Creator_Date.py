'''
Specify in the constants how many datas you want to consider and how many images per data 
...
TOP_DATAS = 5 # Specify how many data will have the new dataset
NUM_IMAGES_PER_DATA = 100 # How many pictures for each datas, specifay "ALL" if you want to take all the pictures
...
def get_top_n_datas(df, n = TOP_DATAS, NUM_IMAGES_PER_DATA = NUM_IMAGES_PER_DATA):
    """
    Returns the rows of the top n datas that appear most frequently in the given DataFrame.

    Parameters:
    - df: DataFrame containing the data after cleaning.
    - n: Number of top datas to select.
    - NUM_IMAGES_PER_DATA: Number of images per data to select. If None, select all images per data.

    Returns:
    - DataFrame: Rows of the top n datas.
    """
    # Count the occurrences of each date
    data_counts = df['date'].value_counts()
    # Get the top n dates
    top_datas = data_counts.head(n).index.tolist()
    
    if NUM_IMAGES_PER_DATA == "ALL":
        # Filter the DataFrame to include only rows with the top n dates
        top_datas_rows = df[df['date'].isin(top_datas)]
    else:
        top_datas_rows = pd.DataFrame()
        for data in top_datas:
            data_rows = df[df['date'] == data]
            top_data_images = data_rows.head(NUM_IMAGES_PER_DATA)
            top_datas_rows = pd.concat([top_datas_rows, top_data_images])
            
    return top_datas_rows
        ...
        for directory, _, files in os.walk(datasets_path):
            ...
            if image_filename in files:
                ...
                if not os.path.exists(destination_path):
                    # Avoid problem of max pixels of the library PIL
                    # Resize the image to 224x224
                    img = Image.open(source_path)
                    try:
                        img_redimensioned = img.resize((224, 224))
                    except OSError as e:
                        print("Error al redimensionar la imagen:", e)
                    
                    if img.mode != 'RGB':
                        img_redimensioned = img_redimensioned.convert('RGB')
                
                    # Save the image
                    img_redimensioned.save(destination_path)
...
# Get the dataset with the top datas
df_top_datas = get_top_n_datas(df_filtered, TOP_DATAS, NUM_IMAGES_PER_DATA)

# Save the dataset
df_top_datas.to_csv(f"TOP{TOP_DATAS}_DATAS_WITH_{NUM_IMAGES_PER_DATA}_PICTURES.csv", index=False)

# Create the new directory
print("\n\n")
destination_directory = f"TOP{TOP_DATAS}_DATAS_WITH_{NUM_IMAGES_PER_DATA}_PICTURES"
copy_images_to_destination_directory(destination_directory, df_top_datas["new_filename"].tolist())
